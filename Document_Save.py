...
def Document_Save(Root_Name, Doc_Name, Layer_Wei, Compress_NT, SP_Dic):
	...
	#层间权重保存
	#觉得应该再保存一个可以查看的.csv
	Layer_write = open(New_Root + '/' + '{}.pkl'.format(Doc_Name + '_' + LW), 'wb')
	pickle.dump(Layer_Wei, Layer_write)
	Layer_write.close()


	#最短路信息保存	
	#觉得应该再保存一个可以查看的.csv
	SP_write = open(New_Root + '/' + '{}.pkl'.format(Doc_Name + '_' + SP_Name), 'wb')
	pickle.dump(SP_Dic, SP_write)
	SP_write.close()
